...
--- `/dop` -- an easter egg. Twenty eddies, once per account, ever.
---
--- Unrestricted: anyone may run it. The protection is not an ACL but the primary key on
--- `opx77_claims`, which is the only thing that can arbitrate two connections of the same
--- account racing each other.
---
--- Keyed on `userId`, not `citizenId`: deleting the character that claimed it and making
--- another must not hand out a second twenty.
--- The claim key, fixed. NOT the command name: renaming the command would make a new key and
--- hand a second twenty to everybody who already took the first.
local EGG_CODE = "dop"

local egg = Config.EASTER_EGG
if type(egg) == "table" and type(egg.COMMAND) == "string" and egg.COMMAND ~= ""
  and (tonumber(egg.REWARD) or 0) > 0 then
  RegisterCommand(egg.COMMAND, function(source, _, raw)
    local src = tonumber(source) or 0
    if src <= 0 then
      return OPX.CommandResult(source, raw, false, "/" .. egg.COMMAND .. " is a player command")
    end
    if OPX.Cooling(src, "easteregg", 5000) then
      return OPX.NotifyLocale(src, "error.tooFast", nil, "error")
    end

    local session = OPX.Sessions[src]
    if not session then
      return OPX.NotifyLocale(src, "entry.noIdentity", nil, "error")
    end
    local player = OPX.GetPlayer(src)
    if not player then
      return OPX.NotifyLocale(src, "error.notLoggedIn", nil, "error")
    end

    CreateThread(function()
      -- the row is written BEFORE the money: a payment that lands and then fails to record
      -- itself is a payment that can be taken again
      local taken = OPX.Storage.Claims.take(session.userId, EGG_CODE,
        player.PlayerData.citizenId)
      if not taken.ok then
        if taken.error == "claim.taken" then
          return OPX.NotifyLocale(src, "egg.already", nil, "error")
        end
        -- the database is unavailable: refuse rather than pay on a claim nothing recorded
        return OPX.NotifyLocale(src, "error.unavailable", nil, "error")
      end

      local reward = math.floor(tonumber(egg.REWARD) or 0)
      if not OPX.AddMoney(src, "EDDIES", reward, "easteregg:" .. EGG_CODE) then
        -- the row is written and the money is not: an operator has to settle this by hand,
        -- so it is said with the account and the amount in it
        OPX.Log.error(("%s claimed %s for %d eddies and the payment failed")
          :format(session.userId, EGG_CODE, reward))
        return OPX.NotifyLocale(src, "error.unavailable", nil, "error")
      end
      OPX.Logger.player(player, "egg.claimed", EGG_CODE, { reward = reward })
      OPX.NotifyLocale(src, "egg.claimed", { amount = reward }, "success")
    end)
  end, false)
end
